...
int LqrPlan::reset(LqrParams params)
{
    m_lqr_params = params;
...
int LqrPlan::execute()
{
    ...
    if (m_phase == 0)
    {
        m_pose_start.x_map = g_robot_status_now.x_map;
        m_pose_start.y_map = g_robot_status_now.y_map;
        m_pose_start.theta_map = g_robot_status_now.theta_map;

        m_pose_end.x_map = m_lqr_params.x_end + m_dist_end*cos(CV_PI+m_lqr_params.theta_end);
        m_pose_end.y_map = m_lqr_params.y_end + m_dist_end*sin(CV_PI+m_lqr_params.theta_end);
        m_pose_end.theta_map = g_robot_status_now.theta_map;
       
        g_robot_rotate_params.theta_target = atan2(m_pose_end.y_map - m_pose_start.y_map, m_pose_end.x_map - m_pose_start.x_map);
    ...
    if (m_phase == 2)
    {
        ...
        _flag = lqrPlanning("whole_path_line", m_pose_start, m_pose_end, pose_now,
                                _v, _omega);
        if (_flag == 0)
        {
            m_phase = 3;
    ...
    if (m_phase == 3)
    {
        g_robot_rotate_params.theta_target = m_lqr_params.theta_end;
        NormalizeAngle(g_robot_rotate_params.theta_target);
        g_robot_controller.setRotateParams(g_robot_rotate_params);
    ...
    if (m_phase == 4)
    {
        ...
        if (_flag == 0)
        {
            ...
            m_pose_start.y_map = g_robot_status_now.y_map;
            m_pose_start.theta_map = g_robot_status_now.theta_map;
            m_pose_end.x_map = m_lqr_params.x_end;
            m_pose_end.y_map = m_lqr_params.y_end;
            m_pose_end.theta_map = m_lqr_params.theta_end;
        }
        if (_flag == -1)
        {
            std::cout<<"docking spin blocking!"<<std::endl;
